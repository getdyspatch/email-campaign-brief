---
name: email-campaign-brief-dyspatch
description: Generates a tight, professional email marketing brief through a guided interview, tailored for teams who design and produce email in Dyspatch.io. Use whenever someone using Dyspatch is planning, scoping, or about to write an email campaign — welcome series, nurture flows, promotional sends, newsletters, product launches, lifecycle/automation, abandonment, or re-engagement. Triggers on phrases like "write a brief for my Dyspatch email", "creative brief for a Dyspatch campaign", "brief an email we'll build in Dyspatch", or "scope a Dyspatch newsletter", and on any email project where Dyspatch is the design/production platform. Treats Dyspatch as the production path and, when the Dyspatch MCP server or API is connected, pulls the account's real modules/blocks so the sample layout references blocks the team can actually use. Also triggers on requests for a sample/blank brief. Works for one email or a sequence; produces Markdown (default), Word, or HTML, and remembers the choice.
metadata:
  version: "2.1.2"
---

# Email Campaign Brief (Dyspatch)

This skill walks a marketer through producing a tight, opinionated email campaign brief for a team that builds and produces email in **Dyspatch.io**. The brief itself follows a deliberately short format — 1.5 pages or less in Word — because long briefs are a strong tell that the strategy isn't actually clear. If you can't get the brief that short, the strategy probably isn't ready.

This is the Dyspatch-tailored variant of the email-campaign-brief skill. It is identical to the generic skill in method and structure, with three differences: it assumes **Dyspatch is the production path** (rather than asking which build path the team uses), it tailors recommendations to what Dyspatch can build, and it grounds the sample layout in the account's **real Dyspatch modules/blocks** when Dyspatch access is connected. All the Dyspatch mechanics — concepts, MCP/API setup, the tools to call, and the fallback when there's no access — live in `references/dyspatch_integration.md`. Read it before Step 3.
...
- **ESP / sending platform** — drives the `utm_source` default. (Dyspatch is the *design/production* layer; the ESP is what actually sends — they're separate, capture both.)
- **Dyspatch setup** — whether the Dyspatch MCP server or API is connected, the default workspace, the active theme, and the modules/blocks this team reaches for most. If a previous session saved these, pull them forward so you can ground the sample layout without re-discovering the account. See `references/dyspatch_integration.md`.
...
- **Preferred output format** — `md` (default), `docx`, or `html`. Honor this in Step 9 without re-asking. If memory is empty, default to `md`.
- **Email-type defaults** — facts about a specific type of email (e.g., "monthly newsletter") that recur across campaigns of that type: typical sender name, standard KPIs and benchmarks, recurring mandatories (footer template, unsubscribe block), default suppression rules, common layout patterns (including which Dyspatch modules they usually use), and any "lessons learned" the user noted previously. These live in per-type memory files (e.g., `email-type-newsletter.md`, `email-type-launch.md`) so the next newsletter brief starts pre-populated. See "Step 10: Save what's worth remembering" for the save format.

When you find applicable memories, **state them back as assumptions for this session and let the user override**. For example:

> Picking up from last time — I'll assume Iterable as your ESP, Dyspatch for production (MCP connected, "Marketing" workspace, "Figma 2026" theme), US+CA+EU compliance scope (CAN-SPAM/CASL/GDPR/UK GDPR/CCPA), and your standard suppression list (paid customers, integrated users, internal domains, sales-engaged). Say "yes" to lock those in, or tell me what's changed.
...
These elements are what makes an email brief different from a generic creative brief. Read `references/email_specifics.md` for the rationale on each, and `references/dyspatch_integration.md` for how the production path works when the team builds in Dyspatch.

**Campaign-level (shared across all emails in the campaign):**
- **Type of message** — welcome / promotional / newsletter / lifecycle / abandonment / re-engagement / transactional / announcement / nurture / other. **Capture this explicitly — it's the key into email-type memory.** If a per-type memory file exists (e.g., `email-type-newsletter.md`), pull its defaults forward as starting assumptions for this brief: sender name pattern, typical KPIs, recurring mandatories, default suppression rules, common layout (including the Dyspatch modules usually used). Surface those to the user so they can confirm or override.
...
- **Localization** — if the campaign is multi-locale, capture: list of locales (en-US, en-GB, fr-FR, etc.), whether each locale needs its own brief / creative concepts or whether one master brief drives translations, per-locale send timing (timezone-aware), per-locale legal/disclosure differences (GDPR for EU, CASL for CA, locale-specific footer requirements), and how the Goal rolls up — is it a single aggregate target, or per-locale targets? Translation rounds typically add 5–10 business days; flag this in the Timeline. (Dyspatch localizes at the module/template level — if access is connected, you can confirm which locales a reused module already supports; see `references/dyspatch_integration.md`.)
- **Production path — Dyspatch.** This variant assumes the team designs and produces in Dyspatch, so you don't ask which build path they use. Instead, confirm it and gather the Dyspatch specifics: "You're building this in Dyspatch — got it. Which **workspace** and **theme** should this live in, and do you want me to pull your account's available **modules/blocks** so the sample layout references real building blocks?" If the Dyspatch MCP server or API is connected, follow the procedure in `references/dyspatch_integration.md` to fetch the workspace's modules/templates (used in Step 7). If it isn't connected, note that and proceed — you'll fall back to a generic block list in Step 7 or ask the user to paste their module names. (If the user says they *don't* actually build in Dyspatch, point them to the generic `email-campaign-brief` skill instead.)
...
- **UTM parameters / link tracking** — **always include this section in the final brief; do not skip it even if the user shrugs.** Ask: "What UTM parameters should we use for links in this campaign? The conventions are usually:
  - `utm_source` — the email sending platform / ESP (Mailchimp / Marketo / Klaviyo / Iterable / HubSpot / whatever sends the mail — not Dyspatch, which is the design layer). **Capture the ESP by name even beyond the UTM** — it also drives the capability-tailored recommendations in Step 6
  ...
  You don't need all five — at minimum capture `utm_source`, `utm_medium`, and `utm_campaign` at the campaign level. `utm_term` and `utm_content` can be set per email/per link." If the user doesn't know or shrugs, **propose sensible defaults based on their ESP and campaign name, present them, and ask the user to confirm or override** — do not leave the UTM block out of the brief. Example fallback for a newsletter sent via HubSpot: `utm_source=hubspot`, `utm_medium=email`, `utm_campaign=spring-newsletter-2026-05`.
...
- **Assets** — ask explicitly, in two parts: **(1)** "What assets already exist that we'll use? (copy drafts, hero imagery, product shots, brand assets, landing pages, video, GIFs, logos, illustrations) — including any **Dyspatch modules** already built that we'll reuse." **(2)** "What assets are net new and need to be created — and by whom (designer, photographer, copywriter, agency) — including any **new Dyspatch modules** that need building?" Capture both lists separately. Net-new assets drive timeline and budget; missing this prompt is the #1 cause of "we found out three days before launch we still need the hero" — so don't skip part (2).
- **Testing & QA** — anything specific to flag (interactive elements, dark mode, accessibility, deliverability concerns, segments to test, Dyspatch preview/proof review steps).
...
Only include budget if external production is involved. Listen for any of these signals in the user's answers:
- They mention an agency, freelancer, designer-for-hire, external developer, or copywriter-for-hire
- They need net-new Dyspatch modules built by someone outside the team, or net-new creative produced externally
...
2. **Research, if web is enabled** — search for: (a) current best-in-class examples of this email type in the user's industry/vertical, (b) recent techniques applicable to this campaign, (c) **capabilities of the user's stack** — both their ESP (captured in Step 3, or recovered from memory in Step 0) *and* Dyspatch. Dyspatch supports modular blocks, DML conditional/dynamic content, AMP and interactive blocks, and module-level localization — recommend techniques the team can actually build in Dyspatch, sent through their ESP. If Dyspatch access is connected, prefer recommendations that reuse or extend modules already in the account (see `references/dyspatch_integration.md`). (d) segmentation studies relevant to the goal. **Consult `references/proven_patterns.md` as a first-class source** alongside web research — prefer a proven, sourced pattern over a freshly-searched claim.
3. **Produce 1–3 recommendations only — not a wall of suggestions.** Each recommendation must be:
   ...
   - **Honest about cost** — if a recommendation adds time, vendors, a new Dyspatch module, or risk, say so

   Example shapes:
   - *"Consider segmenting this newsletter into two sends — paid customers vs. prospects — using a Dyspatch DML conditional block so it's one template, not two builds. For monthly newsletters, segmented sends average ~14% higher CTR ([source]). Adds one QA pass, no new template."*
   - *"For a launch this important, consider a distinct hero — a kinetic CSS-animated block or a before/after — to break pattern with the standard monthly format. If your theme doesn't have that module yet it's a net-new Dyspatch build, ~4–6 hours design + dev."*
   - *"Since the product is visual, consider an inline looping GIF block rather than only linking the video. Inline media for product reveals shows higher engagement than linked video in B2B SaaS ([source])."*
...
### Step 7: Sample layout & email (offer before file output)

Layout and copy are the same artifact at two zoom levels, so the skill produces them together — one block list with draft copy attached to each row. In this Dyspatch variant, the layout is grounded in the account's **real modules/blocks** whenever Dyspatch access is connected.
...
**Ground the layout in real Dyspatch modules.** Follow the procedure in `references/dyspatch_integration.md`:
- If the Dyspatch MCP server or API is connected, call `list_blocks` (and optionally `list_templates`) for the relevant workspace/theme and map each layout row to a real module — show the module **name** and **block id** so the team can drop it straight into Dyspatch. If a row has no matching module, mark it clearly as net-new (it feeds the Assets and Budget sections).
- If Dyspatch access is **not** connected, either ask the user to paste their available module names, or fall back to generic block names — and say which you did. **Never invent Dyspatch module names or block ids.**

**Output shape — a single markdown table** with these columns: `Block | Dyspatch module (id) | Purpose | Suggested treatment | Draft copy`. Rows are the content blocks (Preheader, Header / logo, Hero, Body sections, Primary CTA, Secondary CTA, Footer). Tune the block list to the email type — a newsletter looks different from a launch announcement looks different from a cart abandonment. If Dyspatch access isn't connected, omit or blank the `Dyspatch module (id)` column.
...
- **Brand context** (`user` type) — company name, one-line description, voice guide pointer, recurring proof points the user reuses.
- **ESP and Dyspatch setup** (`user` type) — the sending platform (ESP) and the Dyspatch specifics: that they build in Dyspatch, whether the MCP server/API is connected, default workspace, active theme, and the modules/blocks they reuse most. These rarely change session-to-session and let the next brief skip account discovery. Store the Dyspatch facts in a memory file like `dyspatch-setup.md`. **Do not store API keys or secrets** — only the fact that access is configured and which workspace/theme is default.
...
- **Default owners** (`user` or `project` type) — who runs strategy, design, dev, legal review, deployment.
...
- **Preferred output format** (`user` type) — `md` / `docx` / `html`. Update whenever the user picks a format. Stored in a memory file like `email-brief-output-format.md`.
- **Email-type defaults** (`user` type, one file per type) — facts about a *type* of email (newsletter, launch announcement, abandonment, etc.) that recur across campaigns. Store in files like `email-type-newsletter.md`, `email-type-launch.md`. For each type, capture: typical sender name pattern, baseline KPIs and benchmarks, recurring mandatories, default suppression rules for that type, the common layout/structure the user reuses (**including which Dyspatch modules**), and any "lessons learned" notes. When the same email type comes up again, these defaults pre-populate the interview.
...
- Dyspatch API keys, tokens, or any secret. Save only that access is configured and the default workspace/theme.
...
> Saved to memory for next time: ESP (Iterable), Dyspatch setup (MCP connected, "Marketing" workspace, "Figma 2026" theme), compliance scope (US+CA+EU), standard suppression rules, output format preference (Markdown), email-type defaults for monthly newsletters, and your preference for batched interviews + T+7/T+30 reporting. Let me know if any of that's wrong.
...
- **Dyspatch is the assumed production tool here, so naming it is expected** — but stay neutral about *other* tools (ESPs, asset tools, etc.) unless the user asks. Don't pitch alternatives to Dyspatch; if the user says they don't build in Dyspatch, point them to the generic `email-campaign-brief` skill.
- **Never invent Dyspatch module names or block ids.** If access isn't connected, ask the user or use clearly-labeled generic placeholders — don't fabricate account data.
...
- **Don't generate recommendations and the draft brief in the same turn.** Recommendations are a yes/no/modify gate — present them, stop, wait for the user's decision per rec, *then* draft.
...
- `references/dyspatch_integration.md` — Dyspatch concepts (themes, modules/blocks, templates, localizations), how to connect and use the Dyspatch MCP server / API, the Step 7 procedure for grounding the sample layout in real modules, and the fallback when there's no access.
